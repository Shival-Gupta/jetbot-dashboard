...
/**
 * @brief Sets speeds for all four motors.
 * @param leftSpeed Speed for left motors (-255 to 255)
 * @param rightSpeed Speed for right motors (-255 to 255)
 */
void setMotorSpeeds(int leftSpeed, int rightSpeed) {
...
/**
 * @brief Sets speed for an individual motor.
...
void setMotorSpeed(int index, int speed) {
  ...
  int absSpeed = constrain(abs(speed), 0, PWM_MAX);
  digitalWrite(motors[index].dirPin, (speed >= 0) ? HIGH : LOW);
  analogWrite(motors[index].pwmPin, absSpeed);
}

/**
 * @brief Processes incoming serial data into commands.
 */
void processSerialInput() {
...
/**
 * @brief Handles serial commands to set robot direction.
 * @param command Received command string
 */
void handleCommand(const char* command) {
  if (strcmp(command, "forward") == 0) {
    direction = 1;
    integral = 0; // Reset integral for new direction
