...
def start_service():
    """API endpoint to start the jetbot-dashboard service."""
    try:
...
def stop_service():
    """API endpoint to stop the jetbot-dashboard service."""
    try:
...
def restart_service():
    """API endpoint to restart the jetbot-dashboard service."""
    try:
...
def get_service_status():
    """API endpoint to check the status of the jetbot-dashboard service."""
    try:
        result = subprocess.run(['sudo', 'systemctl', 'is-active', 'jetbot-dashboard.service'], capture_output=True, text=True)
    ...
    <body class="bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-6 min-h-screen">
        <div class="container mx-auto max-w-6xl bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center mb-6">
                <div>
                    ...
                    <div class="flex items-center space-x-2">
                        <!-- <span class="text-gray-700 dark:text-gray-300 text-sm">Service Status:</span> -->
                        <div id="status-dot" class="w-3 h-3 rounded-full"></div>
                        <span id="status-text" class="text-sm text-gray-600 dark:text-gray-400"></span>
                        <!-- <button onclick="updateServiceStatus()" class="py-1 px-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400 dark:bg-gray-600 dark:text-gray-300 dark:hover:bg-gray-500 transition text-sm">Refresh</button> -->
                        <!-- <button onclick="controlService('restart')" class="py-2 px-4 bg-purple-500 text-white rounded hover:bg-purple-600 transition text-sm font-medium">Restart Service</button> -->
                        <button onclick="controlService('restart')" class="py-1 px-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400 dark:bg-gray-600 dark:text-gray-300 dark:hover:bg-gray-500 transition text-sm">Restart Service</button>
                    </div>
                </div>
                <div class="flex items-center space-x-4 mt-4 sm:mt-0">
                    ...
                    <!--
                    <button id="theme-toggle" class="py-1 px-3 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 transition">
                ...
                <div>
                    ...
                    <button onclick="confirmAction('reboot', '/api/system/reboot', 'System is rebooting...')" 
                            class="py-2 px-4 bg-purple-500 text-white rounded hover:bg-purple-600 transition text-sm font-medium">Reboot</button>
                    <button onclick="confirmAction('power off', '/api/system/poweroff', 'System is shutting down...')" 
                            class="py-2 px-4 bg-red-500 text-white rounded hover:bg-red-600 transition text-sm font-medium">Power Off</button>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border border-gray-200 dark:border-gray-700 mb-6">
                ...
                <div>
                    <a href="/arduino-upload" class="py-2 px-4 bg-blue-500 text-white rounded hover:bg-blue-600 transition text-sm font-medium">Arduino Upload</a>
                    <a href="/arduino-serial" class="py-2 px-4 bg-green-500 text-white rounded hover:bg-green-600 transition text-sm font-medium">Serial Monitor</a>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border border-gray-200 dark:border-gray-700">
                    ...
                    {% endif %}
                    <p class="text-sm text-gray-500 dark:text-gray-400">Load Avg (1/5/15m): {{ '%.2f' % analytics.cpu.load_avg[0] }} / {{ '%.2f' % analytics.cpu.load_avg[1] }} / {{ '%.2f' % analytics.cpu.load_avg[2] }}</p>
                </div>

                <!-- Memory -->
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border border-gray-200 dark:border-gray-700">
                    <h2 class="text-xl font-semibold mb-3 text-gray-700 dark:text-gray-300">Memory</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">RAM Usage: {{ analytics.memory.virtual.used_gb }} GB / {{ analytics.memory.virtual.total_gb }} GB ({{ '%.1f' % analytics.memory.virtual.percent }}%)</p>
                    <div class="progress-bar-bg mb-3">
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Swap Usage: {{ analytics.memory.swap.used_gb }} GB / {{ analytics.memory.swap.total_gb }} GB ({{ '%.1f' % analytics.memory.swap.percent }}%)</p>
                    <div class="progress-bar-bg mb-3">
                ...
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border border-gray-200 dark:border-gray-700">
                    ...
                    {% if analytics.disk.partitions %}
                        ...
                        <div class="mb-2">
                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-1 truncate" title="{{ part.device }} mounted at {{ part.mountpoint }} ({{ part.fstype }})">
                    ...
                    {% endif %}
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-3">Total I/O: Read {{ analytics.disk.io.read_mb }} MB / Write {{ analytics.disk.io.write_mb }} MB</p>
                </div>
        ...
        <!-- Uncomment the following line to enable auto-refresh every 7 seconds -->
        <!--
        <script>
            setTimeout(function() {
                location.reload();
